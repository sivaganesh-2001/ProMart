...
const AddProducts = ({ sellerId }) => { // sellerId passed as a prop
  ...
  const uploadImageToFirebase = async (file) => {
    return new Promise((resolve, reject) => {
      ...
      const metadata = { contentType: file.type }; // Set content type for viewing
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      setValue("image", file, { shouldValidate: true }); // Correctly set the file for submission
    } else {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const productData = {
        ...
        sellerEmail: sellerEmail, // Include sellerEmail in product data
      };

      console.log("Product data being email:", sellerEmail);
      ...
      <div className="w-full max-w-6xl p-10 bg-white shadow-lg rounded-lg grid grid-cols-3 gap-8">
        ...
        <div className="col-span-2 ml-10">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-2 gap-6">
            ...
            {/* Description field now comes in place of Image upload */}
        ...
        {/* Image Upload Section (outside the form but still in the grid) */}
