...
function OrderReviewPayment() {
  const navigate = useNavigate();
  const location = useLocation();
  const { shop, cartItems, userAddress, totalMRP, finalTotal } = location.state;
  ...
  //   if (!fetchedAddress?.address || !fetchedAddress?.phone) {
  //     toast.warn("Please add a valid address before confirming the order.");
  //     return;
  //   }
  ...
  //     shopId: shop.id,
  //     sellerId: shop.sellerId, // Assuming sellerId is stored in shop object
  ...
  //     const response = await axios.post("http://localhost:8081/api/orders", orderDetails);
...
return (
  <div className="flex flex-col items-center bg-[#F5F1F7] min-h-screen p-6">
      ...
      <div className="w-full max-w-3xl bg-white p-6 rounded-lg shadow-lg">
          ...
          <p><strong>Name:</strong> {userAddress?.name}</p> {/* Display customer name */}
